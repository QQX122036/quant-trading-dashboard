...
/**
 * ErrorBoundary - 基于 solid-js ErrorBoundary 的错误边界
 *
 * 正确捕获子组件树的 JavaScript 错误，显示错误 UI 并允许恢复。
 * 使用 solid-js 官方的 ErrorBoundary 组件而非 try-catch 模式，
 * 因为 try-catch 无法捕获组件渲染期间抛出的异步/lazy 加载错误。
 */
    ...
    <SolidErrorBoundary
      fallback={(error, reset) =>
        props.fallback ? (
          ...
          <DefaultErrorFallback error={error as Error} reset={reset} info={null} />
        )
      }
    >
      {props.children}
