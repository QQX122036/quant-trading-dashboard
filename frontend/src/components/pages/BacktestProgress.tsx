      ...
      series: [
        {
          ...
          axisLine: {
            lineStyle: {
              width: 8,
              color: [
                [pct / 100, new echarts.graphic.LinearGradient(0, 0, 1, 0, [
          ...
          pointer: { icon: 'circle', length: '60%', width: 6, offsetCenter: [0, '-5%'], itemStyle: { color: '#60A5FA' } },
          axisTick: { distance: -12, length: 4, lineStyle: { color: 'rgba(255,255,255,0.2)', width: 1 } },
          splitLine: { distance: -14, length: 8, lineStyle: { color: 'rgba(255,255,255,0.25)', width: 2 } },
          axisLabel: { color: 'rgba(255,255,255,0.4)', fontSize: 10, distance: 18, formatter: '{value}%' },
          detail: {
            valueAnimation: true,
            formatter: (v: number) => `{a|${v.toFixed(0)}%}`,
            rich: {
              a: { fontSize: 28, fontWeight: 'bold', color: '#60A5FA', fontFamily: 'JetBrains Mono, monospace' },
            },
            offsetCenter: [0, '35%'],
          },
          title: { show: false },
  ...
  return (
    <div class="flex flex-col items-center justify-center gap-4 p-6">
      ...
      <div class={`px-4 py-1.5 rounded-full text-xs font-bold ${
        status() === 'completed' ? 'bg-green-500/20 text-green-400 border border-green-500/40' :
        status() === 'failed' ? 'bg-red-500/20 text-red-400 border border-red-500/40' :
        status() === 'running' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/40' :
