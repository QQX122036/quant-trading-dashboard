...
export const MultiStrategyChart: Component = () => {
  ...
  const buildWeightOption = (
    ...
    series: [{
      ...
      barMaxWidth: 32,
      label: { show: true, position: 'right', formatter: (p: { value: number }) => `${(p.value * 100).toFixed(1)}%`, color: '#9CA3AF', fontSize: 11 },
    }],
  });
  ...
  const buildICOption = (
    ...
    return {
      ...
      xAxis: {
        ...
        data: dates,
        axisLine: { lineStyle: { color: 'rgba(255,255,255,0.1)' } },
        axisLabel: { color: '#9CA3AF', fontSize: 9, formatter: (v: string) => v.slice(5), rotate: 30 },
      },
      yAxis: {
        type: 'value',
        axisLine: { lineStyle: { color: 'rgba(255,255,255,0.1)' } },
      ...
      series: [
        {
          name: 'IC', type: 'line', smooth: true, data: icValues,
          lineStyle: { width: 2, color: '#3B82F6' }, itemStyle: { color: '#3B82F6' },
          areaStyle: { color: new ec.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(59,130,246,0.25)' }, { offset: 1, color: 'rgba(59,130,246,0)' }]) },
          markLine: { silent: true, symbol: 'none', lineStyle: { color: '#6B7280', type: 'dashed', width: 1 }, data: [{ yAxis: 0.03, label: { formatter: '+0.03', color: '#6B7280', fontSize: 9 } }, { yAxis: -0.03, label: { formatter: '-0.03', color: '#6B7280', fontSize: 9 } }] },
        },
        {
          name: 'Rank IC', type: 'line', smooth: true, data: rankValues,
          lineStyle: { width: 1.5, type: 'dashed', color: '#8B5CF6' }, itemStyle: { color: '#8B5CF6' },
        },
      ],
  ...
  const buildContribOption = (
    ...
    legend: {
      ...
      textStyle: { color: '#9CA3AF', fontSize: 10 }, itemWidth: 10, itemHeight: 10,
      formatter: (name: string) => { const item = data.find((d) => d.name === name); return `${name}  ${item ? `${item.value.toFixed(1)}%` : ''}`; },
    },
    series: [{
      type: 'pie', radius: ['40%', '70%'], center: ['35%', '50%'], avoidLabelOverlap: true,
      itemStyle: { borderRadius: 4, borderColor: '#0A0E17', borderWidth: 2 },
      label: { show: true, position: 'outside', formatter: '{b}\n{d}%', color: '#D1D5DB', fontSize: 10 },
      labelLine: { show: true, lineStyle: { color: 'rgba(255,255,255,0.2)' } },
      emphasis: { label: { show: true, fontSize: 12, fontWeight: 'bold' }, itemStyle: { shadowBlur: 12, shadowColor: 'rgba(0,0,0,0.4)' } },
  ...
  const buildEquityOption = (
    ...
    return {
      ...
      series: [{
        type: 'line', smooth: true, data: returns,
        lineStyle: { width: 2, color: '#22C55E' }, itemStyle: { color: '#22C55E' },
        areaStyle: { color: new ec.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(34,197,94,0.25)' }, { offset: 1, color: 'rgba(34,197,94,0)' }]) },
      }],
    };
  };
  ...
  const updateCharts = () => {
    ...
    if (r.factor_weights && weightChart) {
      weightChart.setOption(buildWeightOption(r.factor_weights.map((d, i) => ({ ...d, color: d.color ?? defaultColors[i % defaultColors.length] }))), true);
    }
    if (r.ic_series && icChart) {
      icChart.setOption(buildICOption(r.ic_series), true);
    }
    if (r.factor_contributions && contribChart) {
      contribChart.setOption(buildContribOption(r.factor_contributions.map((d, i) => ({ ...d, color: d.color ?? defaultColors[i % defaultColors.length] }))), true);
    }
    if (r.period_results && equityChart) {
      equityChart.setOption(buildEquityOption(r.period_results as Array<{ date: string; capital: number }>), true);
    }
  };
    ...
    <div class="flex flex-col gap-4 h-full p-4 overflow-hidden">
      <div class="flex items-center justify-between shrink-0">
        <div class="flex items-center gap-3">
          ...
          <Show when={useMock()}>
            <span class="text-xs px-2 py-0.5 rounded bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">演示数据</span>
          </Show>
          <Show when={!useMock() && r()}>
            <span class="text-xs px-2 py-0.5 rounded bg-green-500/20 text-green-400 border border-green-500/30">
              收益: {r()?.total_return?.toFixed(2) ?? 0}% | 夏普: {r()?.sharpe_ratio?.toFixed(2) ?? 0}
            </span>
          </Show>
        </div>
        <button
      ...
      <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-3 shrink-0">
        <div class="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <input type="date" class="bg-[#0A0E17] border border-white/10 rounded px-2 py-1 text-xs" value={startDate()} onInput={(e) => setStartDate(e.target.value)} />
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">结束日期</label>
            <input type="date" class="bg-[#0A0E17] border border-white/10 rounded px-2 py-1 text-xs" value={endDate()} onInput={(e) => setEndDate(e.target.value)} />
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">持仓数</label>
            <input type="number" class="bg-[#0A0E17] border border-white/10 rounded px-2 py-1 text-xs w-16" value={topN()} onInput={(e) => setTopN(Number(e.target.value))} />
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">调仓周期(天)</label>
            <input type="number" class="bg-[#0A0E17] border border-white/10 rounded px-2 py-1 text-xs w-20" value={rebalanceDays()} onInput={(e) => setRebalanceDays(Number(e.target.value))} />
          </div>
          <div class="flex-1">
                ...
                {(f) => (
                  <div class="flex items-center gap-1">
                    ...
                    <input
                      ...
                      onInput={(e) => setFactorWeights((prev) => ({ ...prev, [f.key]: Number(e.target.value) }))}
                    />
                  </div>
                )}
              </For>
      ...
      <Show when={!useMock() && r()}>
        <div class="grid grid-cols-2 sm:grid-cols-6 gap-3 shrink-0">
          <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-3">
            ...
            <div class={`text-lg font-bold ${(r()?.total_return ?? 0) >= 0 ? 'text-red-400' : 'text-green-400'}`}>{r()?.total_return?.toFixed(2) ?? 0}%</div>
          </div>
          <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-3">
            <div class="text-xs text-gray-400">年化收益</div>
            <div class={`text-lg font-bold ${(r()?.annual_return ?? 0) >= 0 ? 'text-red-400' : 'text-green-400'}`}>{r()?.annual_return?.toFixed(2) ?? 0}%</div>
          </div>
          <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-3">
          ...
          <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-3">
            ...
            <div class="text-lg font-bold text-gray-200">{((r()?.final_capital ?? 0) / 10000).toFixed(1)}万</div>
          </div>
        </div>
      </Show>
        ...
        <div class="xl:col-span-3 bg-[#111827]/80 rounded-lg border border-white/10 p-4 min-h-[200px] flex flex-col">
          <div class="flex items-center justify-between mb-2 shrink-0">
            ...
            <div class="flex gap-3 text-xs text-gray-400">
              <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-blue-500 inline-block" /> IC</span>
              <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-purple-500 inline-block" /> Rank IC</span>
            </div>
          </div>
