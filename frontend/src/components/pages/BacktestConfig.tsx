...
export const BacktestConfig: Component<BacktestConfigProps> = (props) => {
  ...
  const loadStrategies = async () => {
    try {
      ...
      setStrategyDefs({
        momentum: { name: '动量策略', description: '追涨杀跌，基于近期收益率惯性', params: { period1: { label: '回望周期', min: 2, max: 60, default: 5, step: 1 }, threshold: { label: '信号阈值', min: 0, max: 0.1, default: 0.0, step: 0.001 }, hold_days: { label: '持仓天数', min: 1, max: 30, default: 5, step: 1 }, position_pct: { label: '仓位比例%', min: 10, max: 100, default: 100, step: 5 } } },
        'dual-ma': { name: '双均线策略', description: 'MA金叉做多，死叉做空', params: { period1: { label: '短均线周期', min: 2, max: 60, default: 5, step: 1 }, period2: { label: '长均线周期', min: 5, max: 120, default: 20, step: 1 }, position_pct: { label: '仓位比例%', min: 10, max: 100, default: 100, step: 5 } } },
        boll: { name: '布林带策略', description: '价格下破下轨买入，上穿上轨卖出', params: { period1: { label: '布林周期', min: 5, max: 60, default: 20, step: 1 }, threshold: { label: '标准差倍数', min: 1.0, max: 3.0, default: 2.0, step: 0.1 }, position_pct: { label: '仓位比例%', min: 10, max: 100, default: 100, step: 5 } } },
        'r-breaker': { name: 'R-Breaker策略', description: '日内突破型策略，支撑阻力反转', params: { threshold: { label: '突破系数', min: 0.1, max: 0.7, default: 0.35, step: 0.05 }, position_pct: { label: '仓位比例%', min: 10, max: 100, default: 100, step: 5 } } },
      });
    }
  };
  ...
  return (
    <div class="flex flex-col gap-4">
      <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-4">
        ...
        <div class="flex gap-2">
          <input
            ...
            aria-label="回测标的股票代码"
          />
          <button class="px-3 py-2 bg-white/10 hover:bg-white/20 rounded text-sm transition-colors" onClick={() => addSymbol(symbolInput())}>添加</button>
        </div>
        <div class="flex flex-wrap gap-1.5 mt-2">
          <For each={['000001.SZ', '000300.SH', '600519.SH', '000858.SZ']}>
            {(s) => (
              <button class="px-2 py-0.5 bg-white/5 hover:bg-white/10 rounded text-xs text-gray-400 transition-colors" onClick={() => addSymbol(s)}>{s}</button>
            )}
          </For>
        </div>
      </div>
      ...
      <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-4">
        ...
        <Show when={currentParams().length > 0} fallback={<div class="text-xs text-gray-500">选择策略后显示参数</div>}>
          <div class="space-y-4">
              ...
              {([key, def]) => (
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-blue-400 font-mono">{formatValue(paramValues()[key] ?? def.default, def.step)}</span>
                  </div>
                  <input
                    type="range"
                    min={def.min}
      ...
      <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs text-gray-400 mb-1" for="bt-start-date">开始日期</label>
            <input id="bt-start-date" type="date" class="w-full bg-[#0A0E17] border border-white/10 rounded px-3 py-2 text-sm" value={startDate()} onInput={(e) => setStartDate(e.target.value)} />
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1" for="bt-end-date">结束日期</label>
            <input id="bt-end-date" type="date" class="w-full bg-[#0A0E17] border border-white/10 rounded px-3 py-2 text-sm" value={endDate()} onInput={(e) => setEndDate(e.target.value)} />
          </div>
        </div>
        <div class="mt-3">
          <label class="block text-xs text-gray-400 mb-1" for="bt-capital">初始资金 (元)</label>
          <input id="bt-capital" type="number" class="w-full bg-[#0A0E17] border border-white/10 rounded px-3 py-2 text-sm" value={initialCapital()} onInput={(e) => setInitialCapital(Number(e.target.value))} aria-label="回测初始资金" />
        </div>
      </div>
      ...
      <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-4">
        <div class="flex justify-between items-center mb-3">
          ...
          <button class="text-xs text-blue-400 hover:text-blue-300 transition-colors" onClick={() => apiActions.fetchBacktestTasks()}>🔄 刷新</button>
        </div>
