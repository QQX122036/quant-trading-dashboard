...
function buildEquityOption(curve: Array<{ date: string; equity: number; benchmark?: number }>): echarts.EChartsOption {
...
function buildDrawdownOption(curve: Array<{ date: string; equity: number }>): echarts.EChartsOption {
  ...
  for (let i = 0; i < drawdowns.length; i++) {
    if (drawdowns[i] < 0 && !inDD) { inDD = true; ddStart = i; }
    else if (drawdowns[i] >= 0 && inDD) {
      inDD = false;
      const dur = i - ddStart;
      if (dur > maxDuration) { maxDuration = dur; maxDDStart = ddStart; }
    }
  }
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        return arr?.length ? `<div style="font-size:11px">${arr[0].axisValue}<br/><span style="color:#EF4444">回撤: ${arr[0].value.toFixed(2)}%</span></div>` : '';
      },
    },
    xAxis: {
      type: 'category',
...
function buildMonthlyOption(curve: Array<{ date: string; equity: number }>): echarts.EChartsOption {
  ...
  const months = Array.from(monthlyMap.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([ym]) => ym);
  ...
  monthlyMap.forEach((v, ym) => {
    ...
    data.push({
      value: [parseInt(ym.slice(5, 7)) - 1, years.indexOf(ym.slice(0, 4)), parseFloat(ret.toFixed(2))],
...
export const BacktestReport: Component<BacktestReportProps> = (props) => {
  ...
  const buildCurve = () => {
    ...
    return r.equity_curve.map((d) => ({ date: d.date, equity: d.equity, benchmark: d.benchmark ?? d.equity }));
  };
  ...
  createEffect(() => {
    ...
    equityChart.setOption(buildEquityOption(c as Array<{ date: string; equity: number; benchmark?: number }>), true);
    drawdownChart.setOption(buildDrawdownOption(c as Array<{ date: string; equity: number }>), true);
  ...
  const metricCards = () => {
    ...
    if (!res) return [];
    return [
      { label: '总收益率', value: `${p(res.total_return) >= 0 ? '+' : ''}${p(res.total_return).toFixed(2)}%`, color: p(res.total_return) >= 0 ? 'text-[#3B82F6]' : 'text-[#22C55E]' },
      { label: '年化收益率', value: `${p(res.annual_return) >= 0 ? '+' : ''}${p(res.annual_return).toFixed(2)}%`, color: p(res.annual_return) >= 0 ? 'text-[#3B82F6]' : 'text-[#22C55E]' },
  ...
  const extraCards = () => {
    ...
    if (!res) return [];
    return [
      { label: '超额收益', value: `${p(res.excess_return) >= 0 ? '+' : ''}${p(res.excess_return).toFixed(2)}%`, color: p(res.excess_return) >= 0 ? 'text-[#3B82F6]' : 'text-[#22C55E]' },
        ...
        <Show when={r()} fallback={
          ...
          <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-4 shrink-0">
            ...
            <div class="grid grid-cols-5 gap-4 text-sm">
              <div><div class="text-xs text-gray-400">标的</div><div class="font-mono mt-0.5">{r()?.ts_code ?? ''}</div></div>
              <div><div class="text-xs text-gray-400">策略</div><div class="mt-0.5">{r()?.strategy_type ?? ''}</div></div>
              <div><div class="text-xs text-gray-400">期初资金</div><div class="tabular-nums mt-0.5">{(r()?.initial_capital ?? 0).toLocaleString()}</div></div>
              <div><div class="text-xs text-gray-400">期末资金</div><div class="tabular-nums mt-0.5">{(r()?.end_capital ?? 0).toLocaleString('en-US', { maximumFractionDigits: 2 })}</div></div>
              <div><div class="text-xs text-gray-400">回测区间</div><div class="text-xs mt-0.5">{r()?.start_date ?? ''} ~ {r()?.end_date ?? ''}</div></div>
            </div>
          </div>
            ...
            <div class="bg-[#111827]/80 rounded-lg border border-white/10 p-4 shrink-0">
              ...
              <div class="overflow-x-auto">
                  ...
                  <tbody>
                    {(r()!.trades ?? []).slice(0, 30).map((t) => (
                      <tr class="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td class={`py-2 pr-4 ${t.direction === '买入' || t.direction === 'long' ? 'text-[#EF4444]' : 'text-[#22C55E]'}`}>{t.direction}</td>
                        <td class="py-2 pr-4 text-right tabular-nums">{t.price.toFixed(2)}</td>
                        <td class="py-2 pr-4 text-right tabular-nums">{t.volume}</td>
                        <td class={`py-2 text-right tabular-nums ${t.pnl !== undefined ? (t.pnl >= 0 ? 'text-[#3B82F6]' : 'text-[#22C55E]') : 'text-gray-600'}`}>
                ...
                <Show when={(r()!.trades?.length ?? 0) > 30}>
                  <div class="text-xs text-gray-500 mt-2">显示前30条，共 {r()!.trades?.length} 条</div>
                </Show>
              </div>
            </div>
          </Show>
