...
import { IChartApi, ISeriesApi, Time, PriceLineOptions, IPriceLine } from 'lightweight-charts';
import {
  Drawing,
  DrawingToolType,
  DrawPoint,
...
export function useChartDrawings(opts: UseChartDrawingsOptions) {
      ...
      if (d.type === 'trendline') {
        ...
        getFibPrices(high, low, fb.levels).forEach(({ level, price }, i) => {
          const line = chart.addLineSeries({
            ...
            lineWidth: level === 0.382 || level === 0.618 ? 2 : 1,
            priceLineVisible: false,
          });
          line.setData([
        ...
        line.setData([
          { time: p1.time, value: high },
          { time: p2.time, value: high },
          { time: p2.time, value: high },
          { time: p2.time, value: low },
          { time: p2.time, value: low },
          { time: p1.time, value: low },
          { time: p1.time, value: low },
          { time: p1.time, value: high },
        ]);
        drawingLines.push({ line });
        ...
        if (alertLineMap.has(al.id)) {
          chart.removeSeries(alertLineMap.get(al.id)!);
          alertLineMap.delete(al.id);
        }
        const line = chart.addLineSeries({
  ...
  function checkAlerts(price: number) {
    ...
    triggered.forEach((a) => {
      ...
      logger.warn(`[Alert] Price ${price} triggered alert at ¥${a.price}`);
    });
    renderDrawings();
  }
  ...
  function deleteDrawing(id: string) {
    ...
    if (al) {
      ...
      if (s) {
        opts.chartRef()?.removeSeries(s);
        alertLineMap.delete(id);
      }
    }
    renderDrawings();
  }
