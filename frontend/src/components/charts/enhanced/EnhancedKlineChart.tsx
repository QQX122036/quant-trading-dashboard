...
export interface ComparedStock {
  ...
  bars: DailyBar[];
  normalizedData: import('lightweight-charts').LineData<Time>[]; // eslint-disable-line
  series?: import('lightweight-charts').ISeriesApi<'Line'>; // eslint-disable-line
}
export interface EnhancedKlineChartProps {
  tsCode?: string; name?: string; bars?: DailyBar[];
...
export const EnhancedKlineChart: Component<EnhancedKlineChartProps> = (props) => {
  ...
  const [rangeSelection, setRangeSelection] = createSignal<{ fromIndex: number; toIndex: number; fromTime: Time; toTime: Time } | null>(null);
  ...
  async function loadData() {
    ...
    try {
      ...
      if (res.code === '0' && res.data?.bars) {
        ...
        setBars(raw); setAdjustedBars(adjustBars(raw, adjustType()));
  ...
  async function loadComparedStock(tsCode: string) {
    try {
      ...
      if (res.code === '0' && res.data?.bars) {
        ...
        setComparedStocks((p) => [...p, { ts_code: tsCode, name, color, bars: adj, normalizedData: normalizeToStart(adj) }]);
      }
    } catch { /* silent */ }
  }

  function handleChartReady(c: IChartApi) { chartRef = c; }
  ...
  function handleChartClick(param: { time?: Time; point?: { x: number; y: number; price: number } }) {
    ...
    if (selectingRange()) { handleRangeSelect(param.time, param.point.price); return; }
    drawing.handleDrawingClick(param.time, param.point.price);
  }

  function handleRangeSelect(time: Time, price: number) {
    if (!selectingRange()) { setRangeStart({ time, price }); setSelectingRange(true); return; }
    const start = rangeStart();
    if (!start) { setSelectingRange(false); return; }
    const all = adjustedBars();
    const fromIdx = all.findIndex((b) => Number(b.trade_date as unknown as number) >= Number(time));
    const toIdx = all.findIndex((b) => Number(b.trade_date as unknown as number) >= Number(start.time));
    ...
    setSelectingRange(false); setRangeStart(null);
  }

  function zoomIn() {
    if (!chartRef) return;
    const ts = chartRef.timeScale(); const vr = ts.getVisibleRange();
    if (!vr) return;
  ...
  onMount(() => { if (!props.bars) loadData(); });

  createEffect(() => {
    if (props.bars && props.bars.length > 0) {
      setBars(props.bars); setAdjustedBars(adjustBars(props.bars, adjustType()));
    }
  });

  createEffect(() => { drawing.renderDrawings(); });

  function getTooltipText(): string | null {
    if (selectingRange()) return '📊 点击K线左键选择区间起点，再点击选择终点';
    if (!drawing.activeTool()) return null;
    const m: Record<string, string> = { trendline: '趋势线', fibonacci: '斐波那契', rectangle: '矩形', text: '文字', alertline: '预警线' };
  ...
  return (
    <div class="relative w-full h-full flex flex-col">
      <ChartControls
        adjustType={adjustType()} onAdjustChange={handleAdjustChange}
        activeTool={drawing.activeTool()} onToolChange={drawing.setActiveTool}
        onZoomIn={zoomIn} onZoomOut={zoomOut}
        selectingRange={selectingRange()} onToggleRange={() => { setSelectingRange(!selectingRange()); setRangeStart(null); }}
        showChips={showChips()} onToggleChips={() => setShowChips(!showChips())}
        showCompare={showCompare()} onToggleCompare={() => setShowCompare(!showCompare())}
        comparedStocks={comparedStocks()}
        onAddComparedStock={(code) => !comparedStocks().some((s) => s.ts_code === code) && loadComparedStock(code)}
        onRemoveComparedStock={(code) => setComparedStocks((p) => p.filter((s) => s.ts_code !== code))}
        visibleCount={visibleCount()} totalCount={totalCount()}
        drawingsCount={drawing.drawings().length} onClearDrawings={drawing.clearAllDrawings}
      />

      <div class="flex-1 flex min-h-0">
        <KLineCanvas
          bars={adjustedBars()} adjustType={adjustType()}
          onCrosshairMove={handleCrosshairMove} onChartReady={handleChartReady}
          onChartClick={handleChartClick} onVisibleRangeChange={(v, t) => { setVisibleCount(v); setTotalCount(t); }}
      ...
      <CompareChart
        show={showCompare()} comparedStocks={comparedStocks()}
        bars={bars()} adjustType={adjustType()}
        onRemoveStock={(code) => setComparedStocks((p) => p.filter((s) => s.ts_code !== code))}
      />

      <RangeStatsPanel stats={rangeStats()} onClear={() => { setRangeSelection(null); setRangeStats(null); }} />
