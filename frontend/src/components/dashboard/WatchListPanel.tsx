...
export const WatchListPanel: Component<{
  ...
  onMount(() => {
    ...
    for (const sym of DEFAULT_WATCHLIST) {
      init[sym] = {
        ts_code: sym,
        name: STOCK_NAMES[sym] || sym.split('.')[0],
        price: 0,
        preClose: 0,
        change: 0,
        changePercent: 0,
  ...
  function SortIcon(props: { col: SortKey }) {
    if (sortKey() !== props.col) return <span class="text-gray-600 ml-1">⇅</span>;
    ...
    return (
      <div class="h-full w-10 flex flex-col items-center py-2 bg-[#111827]/80 border-r border-white/10">
        <button
          ...
          class="text-gray-500 hover:text-white text-xs mb-2"
          title="展开"
        >
          ☰
        </button>
        <For each={DEFAULT_WATCHLIST}>
          {(sym) => (
            <button
              class="w-8 h-8 my-0.5 rounded text-[9px] flex items-center justify-center font-bold"
              style={{ 'writing-mode': 'vertical-rl', 'text-orientation': 'mixed' }}
              classList={{
                'bg-blue-600 text-white': props.selectedSymbol === sym,
                'bg-white/5 text-gray-400': props.selectedSymbol !== sym,
              }}
        ...
        <table class="w-full text-[11px]">
          <thead class="sticky top-0 z-10 bg-[#111827]">
            <tr class="text-gray-500">
              ...
              <th
                class="px-2 py-1.5 text-right font-normal cursor-pointer select-none"
            ...
            <For each={sortedList()}>
              {(tick) => {
                ...
                return (
                  ...
                  >
                    <td class="px-2 py-1.5 tabular-nums text-gray-400 whitespace-nowrap">
                      {tick.ts_code.split('.')[0]}
                    </td>
                    <td
                      class="px-2 py-1.5 font-medium whitespace-nowrap"
                      classList={{ 'text-white': isSelected, 'text-gray-200': !isSelected }}
                    >
                      {tick.name}
                    </td>
                    <td
                      class={`px-2 py-1.5 text-right tabular-nums font-medium ${isUp ? 'text-[#EF4444]' : 'text-[#22C55E]'}`}
                    >
                      {tick.price > 0 ? tick.price.toFixed(2) : '—'}
                    </td>
                    <td
                      class={`px-2 py-1.5 text-right tabular-nums ${isUp ? 'text-[#EF4444]' : 'text-[#22C55E]'}`}
                    >
                    ...
                    <td class="px-2 py-1.5 text-right tabular-nums text-gray-400">
                      {tick.volume > 0
                        ? tick.volume >= 1e8
                          ? (tick.volume / 1e8).toFixed(2) + '亿'
                          : (tick.volume / 1e4).toFixed(2) + '万'
                        : '—'}
                    </td>
                  </tr>
                );
